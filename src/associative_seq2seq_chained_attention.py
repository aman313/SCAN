...
@Model.register("associative_seq2seq_chained_attention")
class AssociativeSeq2SeqChainedAttention(Model):
    ...
    def _prepare_output_projections(self,
        ...
        if self._attention:
            ...
            attended_input_source = self._prepare_attended_input(decoder_hidden, state['encoder_outputs'], state['source_mask'],self._t2t_attention)
            attended_input_ref_source = self._prepare_attended_input(attended_input_source, state['ref_source_encoded'], state['ref_source_mask'],self._attention)
            attended_input_instance_source = self._prepare_attended_input(attended_input_ref_source, state['instance_source_encoded'], state['instance_source_mask'],self._s2s_attention)

            # shape: (group_size, decoder_output_dim + target_embedding_dim)
            decoder_input = torch.cat((attended_input_instance_source, embedded_input), -1)
        else:
            raise NotImplementedError
