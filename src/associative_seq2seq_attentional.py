...
@Model.register("associative_seq2seq_attentional")
class AssociativeSeq2SeqAttentional(Model):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        super(AssociativeSeq2SeqAttentional, self).__init__(vocab)
    ...
    def _encode(self, ref_target_tokens: Dict[str, torch.Tensor],ref_source_tokens: Dict[str, torch.Tensor],instance_source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        instance_ref_vectors = util.weighted_sum(encoded_instance_source, ref_instance_similarity)
...
#         #reduced_encoded_instance_source = torch.sum(encoded_instance_source,dim=1).unsqueeze(1)
#         #embedding_matrix = util.weighted_sum(reduced_encoded_instance_source,reduced_encoded_ref_source.permute(0,2,1))
#         ref_instance_similarity = self._instance_ref_sim(embedded_ref_source,embedded_instance_source)
#         #ref_instance_similarity = -1*ref_instance_similarity
#         #ref_instance_attention = util.masked_softmax(ref_instance_similarity, instance_source_mask)
#         instance_ref_vectors = util.weighted_sum(embedded_instance_source, ref_instance_similarity)
#         instance_ref_vectors = self._source_encoder(instance_ref_vectors,ref_source_mask)
#          
#         #target_instance_ref_similarity = self._target_instref_sim(encoded_ref_target,instance_ref_vectors)
#         #source_target_attention = util.masked_softmax(target_instance_ref_similarity,instance_source_mask)
#         #target_vectors = util.weighted_sum(instance_ref_vectors,source_target_attention)
...
#         }
    ...
    def _prepare_output_projections(self,
        ...
        if self._attention:
            ...
            attended_input = self._prepare_attended_input(decoder_hidden, state['attention_over_src'], state['attention_mask'])
